...
// Helper: login via API to get token — uses .com (email-validator) and 8001 (host alt port)
const API = process.env.API_URL || 'http://localhost:8001';
async function login(request: any) {
  const email = 'trader@kocmatrix.com';
  const pw = 'KocMatrix2025!';
  ...
  await request.post(`${API}/api/v1/auth/register`, {
    data: { email, password: pw, full_name: 'Test' }
  });
  const r2 = await request.post(`${API}/api/v1/auth/login`, {
    data: { email, password: pw }
  });
...
test('trade düzenleme → edit history oluşuyor mu', async ({ page, request }) => {
  ...
  const accRes = await request.get(`${API}/api/v1/accounts`, { headers: { Authorization: `Bearer ${token}` } });
  let accs = await accRes.json();
  let account_id = 1;
  if (Array.isArray(accs) && accs.length) account_id = accs[0].id;
  else {
    const cr = await request.post(`${API}/api/v1/accounts`, {
      headers: { Authorization: `Bearer ${token}` },
      data: { name: 'Test Hesap', base_currency: 'USDT' }
    });
    account_id = (await cr.json()).id;
  }
  const tr = await request.post(`${API}/api/v1/trades`, {
    headers: { Authorization: `Bearer ${token}` },
    data: {
  ...
  // update
  const upd = await request.put(`${API}/api/v1/trades/${trade.id}`, {
    headers: { Authorization: `Bearer ${token}` },
    data: { ...trade, entry_price: 67100, trade_setup_notes: 'updated via e2e' }
  });
  expect(upd.ok()).toBeTruthy();
  // history
  const hist = await request.get(`${API}/api/v1/trades/${trade.id}/history`, {
    headers: { Authorization: `Bearer ${token}` }
  });
...
test('bildirim tetikleme — drawdown/streak', async ({ request }) => {
  const token = await login(request);
  const sum = await request.get(`${API}/api/v1/analytics/summary`, {
    headers: { Authorization: `Bearer ${token}` }
  });
